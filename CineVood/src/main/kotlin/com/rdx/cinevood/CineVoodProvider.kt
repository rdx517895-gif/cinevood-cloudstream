...
class CineVoodProvider : MainAPI() {
    ...
    override suspend fun getMainPage(
        ...
        val slug = request.data  // category slug or "" for latest
        ...
        // DEBUG: if empty, show what happened
        if (items.isEmpty()) {
            try {
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        // e.g. "https://one.1cinevood.watch/o-romeo-2026-amzn-web-dl-hindi-2160p-hevc-ddp-5-1-msub/"
        // → slug = "o-romeo-2026-amzn-web-dl-hindi-2160p-hevc-ddp-5-1-msub"
    ...
    override suspend fun loadLinks(
        ...
        // Get post content HTML from API
    ...
    /** Fetch posts from WP REST API and convert to SearchResponse list */
    private suspend fun fetchPosts(url: String): List<SearchResponse> {
        try {
            val resp = app.get(url, headers = DEFAULT_HEADERS)
            val text = resp.text

            // Check if blocked or not JSON
    ...
    /** Convert a WP API post JSON to SearchResponse */
    private fun postToSearchResponse(post: JSONObject): SearchResponse? {
        ...
        // Get poster from embedded featured media
        ...
        // Fallback: try to find poster in content
        if (poster == null) {
            try {
    ...
    /** Get post HTML content from API (for link extraction) */
    ...
    private fun loadFromHtml(doc: Document, url: String): LoadResponse? {
    ...
    // ══════════════════════════════════════════════════════════════
    //  LINK EXTRACTION (works on both API content and direct HTML)
    // ══════════════════════════════════════════════════════════════
    ...
    private suspend fun extractLinks(
        ...
        // 2 — Download buttons (maxbutton / oxxfle links)
